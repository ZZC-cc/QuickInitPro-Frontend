...
async function searchByTag(tags: string) {
  const res = await getProductsByTagsUsingGet({ tags });
  if (res.code === 200) data.value = res.data;
        ...
        >
          ...
          >
            <template #default>
              <div flex h-40>
                ...
                <div ml>
                  ...
                  <div c-gray-4><span>库存：</span> {{ item.stock }}</div>
            ...
            <template #actions>
              ...
              <li
                style="color: #f84444; font-weight: 550"
                @click="goShopping(item)"
              >
          ...
          />
          <ProductPayModal
            v-if="payModelVisible"
            v-model:visible="payModelVisible"
            :order-id="orderId"
            @ok="hiddenPay"
          />
          <ProductSuccessModal
            v-if="successModelVisible"
            v-model:visible="successModelVisible"
            :order-id="orderId"
            @ok="hiddenSuccess"
          />
        </a-col>
